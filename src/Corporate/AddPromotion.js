...
export default class AddPromotion extends Component {
  ...
  async checkAvailability() {
    ...
    if (
      ...
      await ApiHelper.post(API.checkSlot, params)
        .then(res => {
          if (res == undefined) {
            this.setState({
              loader: false,
  ...
  async bookPromotionSlot() {
    ...
    if (
      ...
      console.log('params Data', params);
      await ApiHelper.post(API.bookPromotionSlot, params)
        .then(res => {
          if (res == undefined) {
            this.setState({
              loader: false,
  ...
  async fetchItems(value) {
    ...
    await ApiHelper.get(url).then(res => {
      ...
      this.arrayholder.map((c, index) => {
        var title = c?.name ? c?.name : c?.title;
        var code = c?.jobId
          ? c?.jobId
          : c?.propertId
          ? c?.propertId
  ...
  async fetchTimeSlots() {
    await ApiHelper.get(API.slots).then(res => {
      this.arrayholder = res.data.data;
      this.setState({
